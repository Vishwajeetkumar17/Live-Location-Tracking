...
const allowedOrigins = process.env.ALLOWED_ORIGINS
  ...
  : ["http://localhost:3000", "http://localhost:7890", "http://localhost:7891"];
...
app.get("/api/locations", auth.verifyToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "Error fetching location history" });
  }
});

// Get all users (for location sharing)
...
app.get("/api/users/:userId/location", auth.verifyToken, async (req, res) => {
  try {
...
// Get all users' latest locations (live tracking)
app.get("/api/locations/live", auth.verifyToken, async (req, res) => {
  try {
    // Consider users "live" if they've shared location within last 5 minutes
    ...
    for (const user of users) {
      const latestLocation = await Location.findOne({
        user_id: user._id,
        timestamp: { $gte: fiveMinutesAgo }, // Only recent locations
      }).sort({ timestamp: -1 });
    ...
    res.status(500).json({ error: "Error fetching live locations" });
  }
});
  ...
  app
    ...
    .on("listening", () => {
      ...
      if (port != process.env.PORT) {
        console.log(
          `NOTE: Remember to update your frontend to use port ${port} instead of ${process.env.PORT}`
        );
      }
    })
    .on("error", (err) => {
