...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://localhost:27017/location-tracker";
...
mongoose
  ...
  .catch((err) => {
    ...
    console.error("----------------");
    console.error(
      'IMPORTANT: Make sure MongoDB is running on your machine. You can start it with "mongod" command.'
    );
    console.error(
      "Location tracking will continue to work, but locations will not be saved."
    );
    console.error("----------------");
  });
...
const userSchema = new mongoose.Schema({
  name: {
    ...
    }, // Only required for password auth
  },
  email: {
    type: String,
    required: true,
